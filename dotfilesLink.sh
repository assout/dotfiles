#!/bin/bash
ln -si ~/Development/dotfiles/.bashrc ~/
ln -si ~/Development/dotfiles/.tmux.conf ~/
ln -si ~/Development/dotfiles/.inputrc ~/
ln -si ~/Development/dotfiles/.gitconfig ~/
ln -si ~/Development/dotfiles/.gitignore ~/
ln -si ~/Development/dotfiles/mdlrc/.mdlrc ~/
ln -si ~/Development/dotfiles/mdlrc/.mdlrc.style.rb ~/
ln -si ~/Development/dotfiles/vim/_vimrc ~/.vimrc
ln -si ~/Development/dotfiles/vim/.vimrc.devlopment ~/Development/vim-plugins
ln -si ~/Development/dotfiles/vim/_gvimrc ~/.gvimrc
ln -si ~/Development/dotfiles/eclipse/_vrapperrc ~/.vrapperrc
ln -si ~/Development/dotfiles/eclipse/_vrapperrc.linux ~/.vrapperrc.local
